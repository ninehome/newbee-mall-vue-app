...
<template>
  <div class="address-box">
    <s-header :name="'編輯收貨地址'" :back="'/user'"></s-header>
    <div class="address-item">
      <van-address-list v-if="from != 'mine'" v-model="chosenAddressId" :list="list" default-tag-text="默認"  add-button-text ="新增地址"    @add="onAdd"
        @edit="onEdit" @select="select"    />
      <van-address-list v-else v-model="chosenAddressId" :list="list"  add-button-text ="新增地址"	     default-tag-text="默認" @add="onAdd"
        @edit="onEdit"  />
    </div>
  </div>
</template>
